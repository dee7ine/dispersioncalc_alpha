...
class UI():
    ...
    _console_frame_layout: []
    _material_frame_layout: []
    _main_frame_layout: []
    ...
    def __init__(self) -> []:
        ...
        self.main_window = sg.Window(title='Counter Strike: Global Offensive', layout=self._main_frame_layout, size=(1250, 750))
        self._app_main_loop()


    def _console_frame_layout(self) -> []:

        return [[sg.Multiline(f"Beginning session {datetime.now().isoformat(' ', 'seconds')}", size=(100, 10), autoscroll=True,
                      reroute_stdout=True, reroute_stderr=True, key='-OUTPUT-', background_color='white')]]


    def _material_frame_layout(self) -> []:
        return [[sg.Text("E [MPa]"), sg.Input('68.9', enable_events=True, key='young_modulus', size=(6, 5))],
    ...
    def _main_frame_layout(self):
        return [[sg.Frame('Lamb waves', layout=[
        [sg.Text('Material'),
         sg.InputCombo(values=self._choices, default_value="AluminumDisperse", key="material_name", enable_events=True,
                       size=(33, 20)), sg.Stretch()],
        [sg.Text('Symmetry modes'),
        ...
        [sg.Button('Calculate and plot', tooltip="Calculate and plot dispersion curves \n for given material data"),
         ...
         sg.Cancel(), sg.Button('Help', tooltip="Helpful tips")]]),
